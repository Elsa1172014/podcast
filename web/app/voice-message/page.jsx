...
export default function VoiceMessage() {
  ...
  const [emotionOverride, setEmotionOverride] = useState(false);
  ...
  const generate = async () => {
    ...
    try {
      const baseSettings = chosen.settings || { similarity_boost: 0.9, stability: 0.55, style: 0.2, speed: 1.0, speaker_boost: true };
      const finalSettings = emotionOverride ? { ...baseSettings, model_id: "eleven_v3", emotion } : baseSettings;
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto", padding: 24 }}>
      ...
      <div style={{ padding: 14, background: "#F5F8F7", borderRadius: 12, marginBottom: 18, borderRight: "4px solid #A63FA6" }}>
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, marginBottom: emotionOverride ? 10 : 0, cursor: "pointer" }}>
          <input type="checkbox" checked={emotionOverride} onChange={(e) => setEmotionOverride(e.target.checked)} />
          🎭 أضِف مشاعر حقيقية لهذه الرسالة (النموذج v3)
        </label>
        {emotionOverride && (
          <select value={emotion} onChange={(e) => setEmotion(e.target.value)} style={{ width: "100%", padding: 10, borderRadius: 8, border: "1px solid #DCE4DF" }}>
