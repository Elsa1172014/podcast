...
export default function NewStandaloneVoice() {
  ...
  const [tuneSettings, setTuneSettings] = useState({ similarity_boost: 0.9, stability: 0.55, style: 0.2, speed: 1.0, pitch: 0, warmth: false, de_ess: false, speaker_boost: true, target_lufs: -16, model_id: "eleven_multilingual_v2", emotion: "none" });
    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: 24 }}>
      ...
      {status === "idle" || status === "failed" ? (
        <>
          ...
          <div style={{ padding: 14, background: "#F5F8F7", borderRadius: 12, marginBottom: 16, borderRight: "4px solid #A63FA6" }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#7A2F7A", marginBottom: 10 }}>🎭 المشاعر داخل الحديث (النموذج v3) — ظاهرة من البداية، تُطبَّق من أول توليد</div>
            <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, marginBottom: tuneSettings.model_id === "eleven_v3" ? 10 : 0, cursor: "pointer" }}>
              <input type="checkbox" checked={tuneSettings.model_id === "eleven_v3"} onChange={(e) => setTuneSettings((s) => ({ ...s, model_id: e.target.checked ? "eleven_v3" : "eleven_multilingual_v2", emotion: e.target.checked ? s.emotion : "none" }))} />
              استعمل النموذج الأحدث (v3) — يدعم مشاعر حقيقية داخل الصوت المُولَّد
            </label>
            {tuneSettings.model_id === "eleven_v3" && (
              <select value={tuneSettings.emotion} onChange={(e) => setTuneSettings((s) => ({ ...s, emotion: e.target.value }))} style={{ width: "100%", padding: 10, borderRadius: 8, border: "1px solid #DCE4DF" }}>
                <option value="none">بلا انفعال محدَّد (طبيعي)</option>
                <option value="excited">متحمّس</option><option value="happy">سعيد</option><option value="sad">حزين</option>
                <option value="calm">هادئ</option><option value="whisper">هامس</option><option value="shout">صارخ</option>
                <option value="curious">فضولي</option><option value="sarcastic">ساخر</option><option value="worried">قلق</option>
                <option value="tired">متعب</option><option value="laughing">ضاحك</option><option value="crying">باكٍ</option>
                <option value="mischievous">مشاكس</option><option value="slow">بطيء متأنٍّ</option>
              </select>
            )}
          </div>
...
function TuneCircles({ settings, onChange, orig, gen }) {
  ...
  return (
    <div>
      ...
      <div style={{ padding: 14, background: "#F5F8F7", borderRadius: 12, borderRight: "4px solid #A63FA6" }}>
        ...
        {settings.model_id === "eleven_v3" && (
          <select className="inp" value={settings.emotion} onChange={(e) => onChange({ ...settings, emotion: e.target.value })} style={{ width: "100%", padding: 10, borderRadius: 8, border: "1px solid #DCE4DF" }}>
            <option value="none">بلا انفعال محدَّد (طبيعي)</option>
            <option value="excited">متحمّس</option><option value="happy">سعيد</option><option value="sad">حزين</option>
            <option value="calm">هادئ</option><option value="whisper">هامس</option><option value="shout">صارخ</option>
            <option value="curious">فضولي</option><option value="sarcastic">ساخر</option><option value="worried">قلق</option>
