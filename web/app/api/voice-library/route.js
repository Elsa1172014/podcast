...
export async function POST(req) {
  try {
    ...
    const workerRes = await fetch(`${process.env.WORKER_URL}/voices/library`, {
      headers: { "x-worker-secret": process.env.WORKER_SECRET },
    });
    const data = await workerRes.json();
    if (!workerRes.ok) return Response.json({ error: data.error || "فشل جلب المكتبة" }, { status: workerRes.status });
