...
export async function POST(req) {
  try {
    ...
    const workerRes = await fetch(`${process.env.WORKER_URL}/cleanup-voices`, {
      method: "POST",
      headers: { "x-worker-secret": process.env.WORKER_SECRET },
    });
    const data = await workerRes.json();
    if (!workerRes.ok) return Response.json({ error: data.error || "فشل التنظيف" }, { status: workerRes.status });
