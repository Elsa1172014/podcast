...
const DEFAULT_SETTINGS = {
  ...
  compress_ratio: 1,     // 1 = بلا ضغط، حتى 10 = ضغط شديد يُصغّر المدى الديناميكي فعليًّا
  de_ess: false,
  warmth: false,
  model_id: "eleven_multilingual_v2", // v2 مستقرّ ومُختبَر أصلًا؛ v3 اختياري (أحدث، يدعم انفعالات حقيقية)
  emotion: "none",
    ...
    <div style={{ maxWidth: 820, margin: "0 auto", padding: 24 }}>
      ...
      {speakers.map((sp) => (
        <SpeakerRow key={sp.id} speaker={sp} sample={samples[sp.id]} session={session} projectId={id}
          ...
          onDelete={async () => {
            if (!confirm(`حذف "${sp.display_name}" نهائيًّا؟ سيُحذَف أي صوت مرتبط به أيضًا.`)) return;
            const supabase = createClient();
            ...
            setSamples((prev) => { const next = { ...prev }; delete next[sp.id]; return next; });
          }} />
      ))}
...
function SpeakerRow({ speaker, sample, session, projectId, open, onToggle, onSaved, onDelete }) {
  return (
    <div style={{ marginBottom: 14, background: "#fff", borderRadius: 10, border: "1px solid #DCE4DF", overflow: "hidden" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 16, flexWrap: "wrap", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button onClick={onDelete} title="حذف هذا المتحدث" style={{ width: 36, padding: "8px 0", borderRadius: 8, border: "1px solid #F0D5D0", background: "#fff", color: "#A6402F", cursor: "pointer", fontSize: 15 }}>🗑</button>
        </div>
      </div>
...
function VoiceStudio({ speaker, sample, session, projectId, onSaved }) {
  ...
  const [schoolLibrary, setSchoolLibrary] = useState(null);
  const loadSchoolLibrary = async () => {
    const supabase = createClient();
    const { data: rows } = await supabase
      .from("voice_samples")
      .select("id, voice_name, voice_model_id, age_stage, description")
      .eq("is_approved", true)
      .not("voice_model_id", "is", null)
      .order("created_at", { ascending: false });
    setSchoolLibrary((rows || []).map((r) => ({ id: r.id, voiceId: r.voice_model_id, name: r.voice_name, description: [r.age_stage, r.description].filter(Boolean).join(" — ") })));
  };
    ...
    <div style={{ padding: 16, borderTop: "1px solid #DCE4DF", background: "#F9FAF9" }}>
      ...
      {showCapture && (
        <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
          ...
          <button onClick={() => { setMode("school"); if (!schoolLibrary) loadSchoolLibrary(); }} style={tabBtn(mode === "school")}>مكتبة المدرسة</button>
          <button onClick={() => { setMode("library"); if (!library) loadLibrary(); }} style={tabBtn(mode === "library")}>مكتبة ElevenLabs</button>
        </div>
      )}

      {mode === "school" && showCapture ? (
        <VoiceLibraryPicker library={schoolLibrary} error="" onPick={(v) => approve(v)} busy={approving}
          onDelete={async (v) => {
            if (!confirm(`حذف صوت "${v.name}" نهائيًّا من مكتبة المدرسة؟`)) return;
            const supabase = createClient();
            const { error } = await supabase.from("voice_samples").delete().eq("id", v.id);
            if (error) return alert("تعذّر الحذف: " + error.message);
            setSchoolLibrary((prev) => prev.filter((x) => x.id !== v.id));
          }} />
        ...
        <>
          ...
          {showCapture && mode !== "library" && mode !== "school" && (
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <p style={{ fontSize: 11, color: "#9AA6A3", margin: "0 0 6px" }}>ضروري لمقارنة حقيقية — إن اختلف النص، ستختلف مدة الكلام ونبرته تلقائيًّا بلا علاقة بجودة الاستنساخ. لأفضل مقارنة، اكتب نصًّا كافيًا لتوليد **40 ثانية فأكثر** من الكلام.</p>
            ...
            <div style={{ padding: 14, background: "#fff", borderRadius: 10, border: "1px solid #DCE4DF", marginBottom: 16 }}>
              ...
              {analysis && (
                <div style={{ marginTop: 14, padding: 16, background: "#fff", borderRadius: 12, border: "1px solid #DCE4DF" }}>
                  <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>نتيجة التحليل الحقيقي</div>

                  {analysis.measured.overall_match_pct != null && (
                    <div style={{ display: "flex", justifyContent: "center", marginBottom: 16 }}>
                      <MatchCircle pct={analysis.measured.overall_match_pct} label="نسبة التطابق الكلية" size={100} />
                    </div>
                  )}
                  ...
                  <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap", marginBottom: 14 }}>
                    {analysis.measured.loudness_match_pct != null && (
                      <MatchCircle pct={analysis.measured.loudness_match_pct} label="مستوى الصوت" size={72} sub={`${analysis.measured.original_lufs?.toFixed(1)} ← ${analysis.measured.generated_lufs?.toFixed(1)} LUFS`} />
                    )}
                    {analysis.measured.pitch_match_pct != null && (
                      <MatchCircle pct={analysis.measured.pitch_match_pct} label="الطبقة الصوتية" size={72} sub={`${analysis.measured.original_f0_hz?.toFixed(0)} ← ${analysis.measured.generated_f0_hz?.toFixed(0)} Hz`} />
                    )}
                    {analysis.measured.dynamic_range_match_pct != null && (
                      <MatchCircle pct={analysis.measured.dynamic_range_match_pct} label="المدى الديناميكي" size={72} sub={`${analysis.measured.original_dynamic_range_db?.toFixed(1)} ← ${analysis.measured.generated_dynamic_range_db?.toFixed(1)} dB`} />
                    )}
                  </div>

                  <ComparisonChart rows={[
                    { key: "lufs", label: "مستوى الصوت", unit: " LUFS", original: analysis.measured.original_lufs, generated: analysis.measured.generated_lufs },
                    { key: "pitch", label: "الطبقة الصوتية", unit: " Hz", original: analysis.measured.original_f0_hz, generated: analysis.measured.generated_f0_hz },
                    { key: "dyn", label: "المدى الديناميكي", unit: " dB", original: analysis.measured.original_dynamic_range_db, generated: analysis.measured.generated_dynamic_range_db },
                  ].filter((r) => typeof r.original === "number" && typeof r.generated === "number")} />

                  <p style={{ fontSize: 11, color: "#9AA6A3", margin: "12px 0 0" }}>{analysis.note}</p>
                </div>
              )}
            </div>
          )}
...
function VoiceSettingsPanel({ settings, onChange }) {
  ...
  const set2 = (key) => (v) => onChange({ ...settings, [key]: v });
  return (
    <div style={{ padding: 18, background: "linear-gradient(180deg, #FBFDFC, #fff)", borderRadius: 14, border: "1px solid #DCE4DF" }}>
      ...
      {/* لوحة الدوائر — نفس أسلوب "استنساخ صوت" بالضبط. لا بيانات تحليل
          أصلية متاحة في هذا التدفّق (لا نُحلّل عينة مصدر هنا)، فتبقى كل
          دائرة بلا حلقة مرجعية خضراء — للضبط اليدوي البصري فقط. */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 8, marginBottom: 16 }}>
        <DualCircleControl label="التشابه والوضوح" min={0} max={1} step={0.05} fmt={(v) => `${Math.round(v * 100)}%`}
          original={null} value={settings.similarity_boost} onChange={set2("similarity_boost")} />
        <DualCircleControl label="ثبات الصوت (إعداد توليد)" min={0} max={1} step={0.05} fmt={(v) => `${Math.round(v * 100)}%`}
          original={null} value={settings.stability} onChange={set2("stability")} />
        <DualCircleControl label="قوة التعبير" min={0} max={1} step={0.05} fmt={(v) => `${Math.round(v * 100)}%`}
          original={null} value={settings.style} onChange={set2("style")} />
        <DualCircleControl label="سرعة الكلام" min={0.7} max={1.2} step={0.01} fmt={(v) => v.toFixed(2)}
          original={null} value={settings.speed} onChange={set2("speed")} />
        <DualCircleControl label="طبقة الصوت" min={-20} max={20} step={0.1} fmt={(v) => `${v > 0 ? "+" : ""}${v}%`}
          original={null} value={settings.pitch} onChange={set2("pitch")} />
        <DualCircleControl label="مستوى الصوت" unit=" LUFS" min={-30} max={-9} step={0.1}
          original={null} value={settings.target_lufs} onChange={set2("target_lufs")} />
      </div>
      <p style={{ fontSize: 11, color: "#9AA6A3", marginTop: -10, marginBottom: 16, textAlign: "center" }}>"التشابه" و"الوضوح" معامل واحد فعليًّا لدى المزوّد — لا تكرار وهمي لمعامل غير موجود.</p>

      {/* بطاقة: التنقية */}
      <div style={{ padding: 16, background: "#F5F8F7", borderRadius: 12, marginBottom: 14, borderRight: "4px solid #7A8B87" }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: "#5B6F6C", marginBottom: 12 }}>🧹 تنقية العيّنة المصدر</div>

        <div style={{ background: "#fff", borderRadius: 10, padding: 14, marginBottom: 14 }}>
          <GradientSlider icon="⏸️" label="طول الوقفات بين الجمل" value={settings.pause_seconds ?? 0.4} min={0.1} max={1.5} step={0.05} onChange={(e) => onChange({ ...settings, pause_seconds: parseFloat(e.target.value) })} fmt={(v) => `${v.toFixed(2)} ثا`} color="#7A8B87" />
          <GradientSlider icon="📉" label="المدى الديناميكي (نسبة الضغط)" value={settings.compress_ratio ?? 1} min={1} max={10} step={0.5} onChange={(e) => onChange({ ...settings, compress_ratio: parseFloat(e.target.value) })} fmt={(v) => v <= 1 ? "بلا ضغط" : `${v.toFixed(1)}:1`} color="#7A8B87" />
        </div>

        <div style={{ marginBottom: 14 }}>
          <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: "#5B6F6C", marginBottom: 8 }}>تقليل الضوضاء</label>
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {[["off", "متوقف"], ["light", "خفيف"], ["medium", "متوسط"], ["strong", "قوي"]].map(([k, l]) => (
              <button key={k} onClick={() => onChange({ ...settings, noise_reduction: k })}
                style={{ flex: "1 1 auto", minWidth: 70, padding: "8px 14px", borderRadius: 8, border: settings.noise_reduction === k ? "none" : "1px solid #DCE4DF", cursor: "pointer", fontSize: 12, fontWeight: 600,
                  background: settings.noise_reduction === k ? "#5B6F6C" : "#fff", color: settings.noise_reduction === k ? "#fff" : "#14201E" }}>{l}</button>
            ))}
          </div>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, background: "#fff", borderRadius: 10, padding: 14 }}>
        ...
        <p style={{ fontSize: 11, color: "#9AA6A3", margin: "10px 2px 0" }}>حدّ تقني بشأن "إزالة الصدى": لا تتوفّر إزالة صدى حقيقية بلا نموذج ذكاء اصطناعي متخصّص؛ هذا مرشّح تقريبي فقط.</p>
      </div>
      ...
      {/* بطاقة: المشاعر الحقيقية — v3 */}
      <div style={{ padding: 14, background: "#F5F8F7", borderRadius: 12, marginTop: 14, borderRight: "4px solid #A63FA6" }}>
        <div style={{ fontSize: 12, fontWeight: 700, color: "#7A2F7A", marginBottom: 10 }}>🎭 المشاعر داخل الحديث (النموذج v3)</div>
        <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 13, marginBottom: settings.model_id === "eleven_v3" ? 12 : 0, cursor: "pointer" }}>
          <input type="checkbox" checked={settings.model_id === "eleven_v3"} onChange={(e) => onChange({ ...settings, model_id: e.target.checked ? "eleven_v3" : "eleven_multilingual_v2", emotion: e.target.checked ? settings.emotion : "none" })} />
          استعمل النموذج الأحدث (v3) — يدعم مشاعر حقيقية داخل الصوت المُولَّد
        </label>
        {settings.model_id === "eleven_v3" && (
          <>
            <select className="inp" value={settings.emotion} onChange={(e) => onChange({ ...settings, emotion: e.target.value })} style={{ width: "100%", padding: 10, borderRadius: 8, border: "1px solid #DCE4DF", marginBottom: 8 }}>
              <option value="none">بلا انفعال محدَّد (طبيعي)</option>
              <option value="excited">متحمّس</option>
              <option value="happy">سعيد</option>
              <option value="sad">حزين</option>
              <option value="calm">هادئ</option>
              <option value="whisper">هامس</option>
              <option value="shout">صارخ</option>
              <option value="curious">فضولي</option>
              <option value="sarcastic">ساخر</option>
              <option value="worried">قلق</option>
              <option value="tired">متعب</option>
              <option value="laughing">ضاحك</option>
              <option value="crying">باكٍ</option>
              <option value="mischievous">مشاكس</option>
              <option value="slow">بطيء متأنٍّ</option>
            </select>
            <p style={{ fontSize: 11, color: "#9AA6A3", margin: 0 }}>
              حقيقي فعليًّا — الانفعال يُحقَن كوسم مباشر داخل النص المُرسَل لـElevenLabs v3 (لا معالجة صوتية بديلة). حدّان صادقان: (1) v3 أحدث وقد يختلف استقراره قليلًا عن v2 المُختبَر أصلًا في هذا المشروع؛ (2) الانفعال هنا واحد لكل توليد كامل، لا متغيّرًا سطرًا بسطر داخل نفس المقطع — لتنويع الانفعال جملة بجملة نحتاج تعديلًا إضافيًّا لاحقًا في محرّر السيناريو نفسه.
            </p>
          </>
        )}
      </div>
...
// دائرة تطابق ملوَّنة: أخضر لتطابق مرتفع، أصفر لفرق متوسط، أحمر لفرق كبير —
// نفس نظام الألوان المطلوب صراحة.
function MatchCircle({ pct, label, size = 80, sub }) {
  const color = pct >= 75 ? "#1D8348" : pct >= 45 ? "#D4A017" : "#DC2626";
  const inner = size - 14;
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
      <div style={{ width: size, height: size, borderRadius: "50%", background: `conic-gradient(${color} ${pct * 3.6}deg, #E7ECEA 0deg)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: inner, height: inner, borderRadius: "50%", background: "#fff", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <span style={{ fontWeight: 800, fontSize: size > 90 ? 20 : 15, color }}>{pct}%</span>
        </div>
      </div>
      <span style={{ fontSize: 12, fontWeight: 600, textAlign: "center" }}>{label}</span>
      {sub && <span style={{ fontSize: 10, color: "#9AA6A3" }}>{sub}</span>}
    </div>
  );
}

// رسم بياني حقيقي (أعمدة أفقية) — يقارن القيمة الفعلية المقاسة من العيّنة
// الأصلية بالقيمة الفعلية المقاسة من المُولَّد، لكل معيار بمقياسه الخاص.
function ComparisonChart({ rows }) {
  if (!rows?.length) return null;
  return (
    <div style={{ marginTop: 14, padding: 16, background: "#F9FAF9", borderRadius: 10, border: "1px solid #DCE4DF" }}>
          ...
          return (
            <div key={r.key}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                ...
                <div style={{ flex: 1, height: 14, background: "#EDF1EF", borderRadius: 7, overflow: "hidden" }}>
                  <div style={{ width: `${origPct}%`, height: "100%", background: "#14746F", borderRadius: 7, transition: "width .4s" }} />
                </div>
                <span style={{ fontSize: 11, fontWeight: 700, width: 60, textAlign: "left" }}>{r.original.toFixed(1)}{r.unit}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 11, fontWeight: 700, width: 60, textAlign: "left" }}>{r.generated.toFixed(1)}{r.unit}</span>
              </div>
            </div>
          );
        })}
...
function VoiceLibraryPicker({ library, error, onPick, busy, onDelete }) {
      ...
      {library.map((v) => (
        <div key={v.voiceId} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 10, background: "#fff", borderRadius: 8, border: "1px solid #DCE4DF" }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            ...
            {v.id && onDelete && (
              <button onClick={() => onDelete(v)} title="حذف من مكتبة المدرسة" style={{ width: 36, borderRadius: 8, border: "1px solid #F0D5D0", background: "#fff", color: "#A6402F", cursor: "pointer" }}>🗑</button>
            )}
          </div>
        </div>
      ))}
