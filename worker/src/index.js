...
import { produceEpisode, humanizeEpisode } from "./produceEpisode.js";
import { createClient } from "@supabase/supabase-js";
import { textToSpeech, cloneVoice, getVoiceLibrary, deleteVoice, listClonedVoices, transcribeAudio } from "./services/elevenlabs.js";
import { applyPitch, applyLoudnessAndCompression, applyDeEssing, applyPauseLength, applyWarmth, preprocessSample, getDuration, measureLoudnessLUFS, estimateF0, analyzeVoiceSample, measureDynamicsAndNoise } from "./services/audioPost.js";
...
// "معالجة الحلقة" — فحص جودة حقيقي وإعادة توليد انتقائية، ثم توحيد صوت
// نهائي على الحلقة كاملة. غير متزامن كـ/produce تمامًا لنفس السبب.
app.post("/humanize-episode", async (req, res) => {
  if (!checkSecret(req, res)) return;
  const { projectId } = req.body || {};
  if (!projectId) return res.status(400).json({ error: "projectId مطلوب" });

  res.status(202).json({ accepted: true });
  humanizeEpisode(projectId).catch((e) => console.error("فشلت معالجة الحلقة:", projectId, e.message));
});
...
// تفريغ نصّي حقيقي للعيّنة الأصلية — يُستعمل لملء نص التجربة تلقائيًّا
// بدل الكتابة اليدوية، لضمان نص مطابق فعليًّا لما قيل في التسجيل.
app.post("/transcribe", async (req, res) => {
  if (!checkSecret(req, res)) return;
  const { audioBase64, mimeType } = req.body || {};
  if (!audioBase64) return res.status(400).json({ error: "الصوت المطلوب تفريغه مفقود" });
  try {
...
app.post("/preview-voice", async (req, res) => {
  ...
  // لا حدّ على طول النص هنا — المستخدم يتحكّم بالمدة عبر طول ما يكتبه؛
  // نصّ أطول = مقارنة أدق (40 ثانية فأكثر مُوصى بها).
  ...
  try {
    ...
    const audioBuffer = await textToSpeech({ text: trimmedText, voiceId: finalVoiceId, settings, modelId: settings?.model_id || "eleven_multilingual_v2", emotion: settings?.emotion || null });
    ...
    outPath = await applyLoudnessAndCompression(outPath, { targetLUFS: settings?.target_lufs ?? -16, compressRatio: settings?.compress_ratio ?? 1 });
...
app.post("/analyze-match", async (req, res) => {
  ...
  try {
    ...
    const [origLUFS, genLUFS, origDur, genDur, origF0, genF0, origDyn, genDyn] = await Promise.all([
      ...
      measureDynamicsAndNoise(origPath).catch(() => null), measureDynamicsAndNoise(genPath).catch(() => null),
    ]);
    ...
    // المدى الديناميكي — مقياس astats حقيقي فعّال هنا لأول مرة في هذه
    // النقطة تحديدًا، لا زرًّا وهميًّا. الفارق بين المدَيين يُترجَم لنسبة
    // تطابق حقيقية مباشرة.
    let dynamicMatchPct = null;
    const origDR = origDyn?.dynamicRangeDb, genDR = genDyn?.dynamicRangeDb;
    if (typeof origDR === "number" && typeof genDR === "number") {
      dynamicMatchPct = Math.max(0, Math.min(100, Math.round(100 - Math.abs(origDR - genDR) * 8)));
    }

    // نسبة تطابق كلية حقيقية — متوسط كل الأبعاد المقاسة فعليًّا فقط (لا
    // إدراج أي بُعد "غير متاح" ضمن الحساب).
    const parts = [loudnessMatchPct, pitchMatchPct, dynamicMatchPct].filter((v) => v != null);
    const overallPct = parts.length ? Math.round(parts.reduce((a, b) => a + b, 0) / parts.length) : null;
    ...
    res.json({
      measured: {
        ...
        original_dynamic_range_db: origDR ?? null, generated_dynamic_range_db: genDR ?? null,
        loudness_match_pct: loudnessMatchPct,
        pitch_match_pct: pitchMatchPct,
        dynamic_range_match_pct: dynamicMatchPct,
        overall_match_pct: overallPct,
      },
      recommendedSettings: {
      ...
      note: "المقاسات الحقيقية هنا: مستوى الصوت (LUFS)، المدة، التردد الأساسي (F0)، والمدى الديناميكي (astats). هذا ليس بصمة هوية متحدث كاملة — تلك تحتاج نموذج ذكاء اصطناعي متخصّص غير متاح هنا، فلا نعرض لها رقمًا.",
    });
  } catch (e) {
    res.status(500).json({ error: String(e.message || e) });
  } finally {
