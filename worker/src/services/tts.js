...
export async function synthesizeSegment({ text, voiceModelId, outDir, segmentId, settings }) {
  ...
  const audioBuffer = await textToSpeech({ text, voiceId: voiceModelId, settings, modelId: settings?.model_id || "eleven_multilingual_v2", emotion: settings?.emotion || null });
  await fs.writeFile(outPath, audioBuffer);
  let tunedPath = outPath;
  tunedPath = await applyPitch(tunedPath, settings?.pitch || 0);
  tunedPath = await applyLoudnessAndCompression(tunedPath, { targetLUFS: settings?.target_lufs ?? -16, compressRatio: settings?.compress_ratio ?? 1 });
